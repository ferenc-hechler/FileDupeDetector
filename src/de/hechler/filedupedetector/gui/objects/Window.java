...
public class Window extends JFrame {
	...
	public Window load() {
		...
		changeSearchFolderButton = new MenuButton().load(X_1, Y_1, new ImageIcon("./icons/changeSearchFolder.png"),
				a -> changeSearchFolder());
		...
		delete = new MenuButton().load(X_2 + VOID + MenuButton.SIZE + VOID + MenuButton.SIZE, Y_1,
				new ImageIcon("./icons/deleteMarked.png"), a -> delete());
		add(delete);
		add(goOut);
		add(down);
		add(up);
		...
		for (int i = 0; i < goIn.length; i++) {
			final int index = i;
			goIn[i] = new SmallButton().load(X_1, Y_2 + SmallButton.HEIGHT * (i + 1), new ImageIcon("./icons/goIn.png"),
					e -> goIn(index));
			add(goIn[i]);
